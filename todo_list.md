...
## 4. Support for ROCm Systems Profiler
...
ROCm Systems Profiler is the only real target here. AMD GPUs dominate the current TOP500 (Frontier, El Capitan, LUMI). QUDA supports HIP. rocprof output formats (CSV, JSON, and SQLite-like format in ROCm 6+) are more tractable than most alternatives. Conceptual mapping to Nsight's data model is close: kernels, dispatches, memory transfers, hardware counters.

Intel VTune (the analogous profiler for Intel accelerators) uses a proprietary database, which means it will not be worth the effort to include. Blame them, not me.

### Main difficulties

- **Format diversity**: Nsight's SQLite is unusually convenient to query directly. ROCm Systems Profiler can output to sqlite, but will have different tables, etc.
...
Probably we should do a targeted refactoring of the code before trying to implement. First, need to obtain a ROCm profile to explore.
...
A `--profiler {nsight,rocprof}` flag on the CLI selects the adapter. Auto-detection from file extension is a nice-to-have.
